...
def categorias_crescimento_desde_2015(
    ...
    """
    Calcula a taxa de crescimento anual (a.a.) por categoria no período fixo start_year → último ano.
    Regra:
      - Se o valor em start_year > 0: usa CAGR clássico entre start_year e último ano.
      - Se o valor em start_year == 0: usa tendência logarítmica (slope de ln(valor+ε) vs ANO).
        ε = 1% da mediana dos valores positivos da série da categoria (ou 1.0 se não houver mediana).
    ...
    for cat, g in anuais.groupby(col_cat):
        ...
        # reindexa a série para TODOS os anos de start_year até o último ano do dataset da categoria
        y0 = int(start_year)
        y1 = int(g["ANO"].max())
        anos_range = list(range(y0, y1 + 1))
        full = (
            pd.DataFrame({"ANO": anos_range})
        ...
        if v0 > 0:
            ...
            # fallback robusto: slope de ln(valor+ε) vs ANO
            serie_pos = full["VALOR_ANO"][full["VALOR_ANO"] > 0]
            eps = float(np.median(serie_pos) * 0.01) if not serie_pos.empty else 1.0
            y = np.log(full["VALOR_ANO"] + eps)
            x = full["ANO"].astype(float)
            # slope (por ano)
        ...
        out.append({
            ...
            "ANOS": anos,
            "METODO": metodo,
            "CRESC_AA_%": round(float(cresc) * 100.0, 2),
        })
