...
tab1, tab2 = st.tabs(["📈 Indicadores de Vendas", "🏷️ Fornecedores"])

with tab1:
    col1, col2 = st.columns(2)
    with col1:
        st.subheader("Fornecedor TOP (10 anos) – RJ/SP")
        st.dataframe(fornecedor_top_por_uf(df_erp, anos=10))
    with col2:
        st.subheader("Fornecedor TOP (2 anos) – RJ/SP")
        st.dataframe(fornecedor_top_por_uf(df_erp, anos=2))

    c1, c2 = st.columns(2)
    with c1:
        st.subheader("🏆 Maior OF")
        st.dataframe(maior_ordem_fornecimento(df_erp))
    with c2:
        st.subheader("📉 Menor OF")
        st.dataframe(menor_ordem_fornecimento(df_erp))

    media, detalhado = valor_medio_por_of(df_erp)
    st.metric("Valor médio por OF", _format_brl(media))
    with st.expander("OFs com total"):
        st.dataframe(detalhado)
    ...
    st.dataframe(periodo_maior_volume_bimestre(df_erp, anos=10))

    st.subheader("Mês com maior volume (último ano)")
    st.dataframe(mes_maior_volume_ultimo_ano(df_erp))
